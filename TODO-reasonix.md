...
## Harness Engineering（讨论日期：2026-06-12）

- [ ] **熵管理 — /cleanup 命令** — 定期清理项目中的垃圾文件（死代码、空日志、旧 checkpoint、无用的测试文件）
  - 做法：扫描项目目录 → 列出可疑文件 → 用户确认后删除
  - 参考 OpenAI 做法：专门的 Agent 扫描偏差，但起步阶段用斜杠命令即可
  - 注意：清理操作本身有风险，应先列清单后确认，不直接删
