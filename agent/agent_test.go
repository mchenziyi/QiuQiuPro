...
func TestIsHighRiskTool(t *testing.T) {
	high := []string{"write_file", "edit_file_block", "run_shell", "run_powershell"}
	for _, name := range high {
		if !IsHighRiskTool(name) {
			t.Errorf("%s 应被判为高危", name)
		}
	}
	safe := []string{"read_file", "list_directory", "glob", "grep", "search_files", "count_file_chars", "git_commit", "unknown_tool", ""}
